...
from ml.datasets.multiview_real_dataset import MultiViewRealDataset, N_MFCC
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, raw_out, mfcc_out, ssl_out):
        stacked = torch.stack([raw_out, mfcc_out, ssl_out], dim=1)
...
class MultiViewModel(nn.Module):
    ...
    def forward(self, raw, mfcc, ssl):
        return self.fusion(
            self.raw_branch(raw),
            self.lfcc_branch(mfcc),
            self.ssl_branch(ssl)
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Training fusion model on {train_size} real samples, "
        f"testing on {test_size}..."
    )
    ...
    for epoch in range(10):
        ...
        for raw, mfcc, ssl, labels in train_loader:
            optimizer.zero_grad()

            logits, _ = model(raw, mfcc, ssl)
    ...
    with torch.no_grad():
        for raw, mfcc, ssl, labels in test_loader:
            logits, _ = model(raw, mfcc, ssl)
