...
class ASVspoofDataset(Dataset):
    def __init__(self, protocol_path, audio_dir, n_lfcc=20, max_samples=None):
        self.audio_dir = audio_dir
        self.n_lfcc = n_lfcc
        self.samples = parse_protocol_file(protocol_path)
        if max_samples:
            self.samples = self.samples[:max_samples]  # useful for quick testing on a subset
...
if __name__ == "__main__":
    ...
    if not os.path.exists(PROTOCOL_PATH):
        ...
        # max_samples=5 so this test runs instantly instead of loading the whole dataset
